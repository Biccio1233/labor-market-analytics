...
DB_HOST = '127.0.0.1'           # Il tuo host del database
DB_NAME = 'Eurostat'              # Il nome del tuo database
DB_USER = 'postgres'              # Il tuo nome utente del database
DB_PASSWORD = '1233PippoFra!?'     # La tua password del database
DB_PORT = '5432'                  # La tua porta del database
...
def create_database_if_not_exists():
    ...
    conn.set_isolation_level(ISOLATION_LEVEL_AUTOCOMMIT)  # Necessario per creare database
    cursor = conn.cursor()
...
def create_download_logs_table(engine):
    ...
    try:
       ...
       # logger.info(f"Esecuzione della query SQL: {create_table_query}")  # Log della query
        ...
        logger.error(f"Errore durante la creazione della tabella 'download_logs': {e}")
        traceback.print_exc()
...
def drop_table_if_exists(table_name, engine):
    ...
    with engine.begin() as connection:
        ...
        logger.info(f"Tabella '{table_name}' e le sue viste associate sono state cancellate.")
...
def dataframe_to_postgres(dataframe, table_name, engine):
    ...
    logger.info(f"Tabella '{table_name}' creata e popolata con successo in PostgreSQL.")
...
def is_dataset_up_to_date(dataset_code, engine):
    ...
    try:
        ...
        with engine.connect() as connection:
            eurostat_result = connection.execute(query_eurostat, {'dataset_code_upper': dataset_code_upper}).fetchone()
            logs_result = connection.execute(query_logs, {'dataset_code_upper': dataset_code_upper}).fetchone()
            ...
            if eurostat_result:
                ...
                if isinstance(last_table_structure_change, str):
                    last_table_structure_change = parser.isoparse(last_table_structure_change)
                ...
                if logs_result:
                    ...
                    if isinstance(last_download_date, str):
                        last_download_date = parser.isoparse(last_download_date)
                    ...
                    if (last_update_of_data and isinstance(last_update_of_data, datetime)):
                        last_update_of_data = last_update_of_data.date()

                    if (last_table_structure_change and isinstance(last_table_structure_change, datetime)):
                    ...
                    if (last_update_of_data and last_download_date >= last_update_of_data) and \
                       (last_table_structure_change and last_download_date >= last_table_structure_change):
                        logger.info(f"Il dataset '{dataset_code_upper}' è già aggiornato. Nessun download necessario.")
                        return True
                    else:
                        logger.info(f"Il dataset '{dataset_code_upper}' non è aggiornato rispetto a Eurostat. Procedo con il download.")
                        return False
                else:
                    logger.info(f"Nessuna data di scaricamento trovata per il dataset '{dataset_code_upper}'. Procedo con il download.")
                    return False
            else:
                logger.warning(f"Dataset '{dataset_code_upper}' non trovato nella tabella 'eurostat_datasets'.")
                return False
    except Exception as e:
        logger.error(f"Errore durante la verifica dell'aggiornamento del dataset '{dataset_code}': {e}")
        traceback.print_exc()
...
def update_last_download_date(dataset_code, engine):
    ...
    try:
        current_date = datetime.now().date()
        dataset_code_upper = dataset_code.upper()  # Convertiamo il codice in maiuscolo
        ...
        with engine.begin() as connection:
            connection.execute(query, {'dataset_code_upper': dataset_code_upper, 'current_date': current_date})
       
        logger.info(f"Data di scaricamento aggiornata per il dataset '{dataset_code_upper}' nella tabella 'download_logs'.")
    except Exception as e:
        logger.error(f"Errore durante l'aggiornamento della data di scaricamento per il dataset '{dataset_code}': {e}")
        traceback.print_exc()
...
def create_view_with_codelists(dataset_code, dataframe, engine):
    ...
    """
    try:
        pars = get_pars(dataset_code)  # Ottieni i parametri (dimensioni) del dataset
        logger.info(f"Parametri ottenuti per il dataset '{dataset_code}': {pars}")
        ...
        for par in pars:
            if par in dataframe.columns:
                ...
                if inspector.has_table(codelist_table_name):
                    select_columns.append(f'"{codelist_table_name}".description AS {par}_description')
                ...
                logger.warning(f"Parametro '{par}' non presente nel dataframe, ignorato.")
        ...
        logger.error(f"Errore durante la creazione della vista per il dataset '{dataset_code}': {e}")
        traceback.print_exc()
...
def create_normalized_view_with_codelists(dataset_code, dataframe, engine):
    ...
    try:
        ...
        time_column_array = "ARRAY[" + ", ".join([f"'{col}'" for col in time_columns]) + "]"
        value_array = "ARRAY[" + ", ".join([f'{dataset_table_name}."{col}"' for col in time_columns]) + "]"
        ...
        for par in pars:
            ...
            inspector = inspect(engine)
            if inspector.has_table(codelist_table_name):
                join_clauses.append(f'LEFT JOIN {codelist_table_name} ON {dataset_table_name}.{par} = {codelist_table_name}.code')
                select_columns.append(f'{codelist_table_name}.description AS {par}_description')
            else:
                logger.warning(f"Nessuna codelist trovata per il parametro '{par}' nel dataset '{dataset_code}'.")
        ...
        logger.info(f"Creazione della vista normalizzata per il dataset '{dataset_code}': {view_name}")
        ...
        logger.error(f"Errore durante la creazione della vista normalizzata per il dataset '{dataset_code}': {e}")
        traceback.print_exc()
...
def download_and_save_dataset(node, engine):
    ...
    try:
        ...
        create_view_with_codelists(node['code'], dataframe, engine)  # Correggi il richiamo alla funzione
        create_normalized_view_with_codelists(node['code'], dataframe, engine)
    except Exception as e:
        print(f"Errore durante il download o l'inserimento del dataset '{node['code']}': {e}")
        traceback.print_exc()
        ...
        if pars:
            for par in pars:
                ...
                if dic_df is not None and not dic_df.empty:
                    ...
                    print(f"Codelist per il parametro '{par}' salvata nella tabella '{codelist_table_name}'.")
                else:
                    print(f"Nessuna codelist trovata per il parametro '{par}'.")
        else:
            print(f"Nessun parametro trovato per il dataset '{dataset_code}'.")
    except Exception as e:
        print(f"Errore durante il download delle codelist per il dataset '{dataset_code}': {e}")
        traceback.print_exc()
    ...
    while True:
        if node['type'] == 'branch':
            ...
            if is_dataset_up_to_date(node['code'], engine):
                ...
                download_choice = input(f"Vuoi scaricare il dataset '{node['name']}' e le sue codelist? (yes/no): ").strip().lower()
